...
/*
  ...
  #include "LangloEEPROMchar.h"     // Defines the payload Struct and EEPROM locations of specific parameters
#else
  #include "LangloEEPROM.h"     // Defines the payload Struct and EEPROM locations of specific parameters
#endif
*/
#include "LangloEEPROMuint.h"     // Defines the payload Struct and EEPROM locations of specific parameters
...
Serial.print("[eepromReadBytes] HByte : ");
Serial.println((int)(eepromAddress >> 8),HEX);
Serial.print("[eepromReadBytes] LByte : ");
Serial.println((int)(eepromAddress & 0xFF),HEX);
  ...
  uint16_t lp = 2;
  Wire.requestFrom(I2C_EEPROM_Address, lp); delay(10);
  
  uint8_t rdata;
  for (int x = 0 ; x < byteCount ; x++) {        
    if (Wire.available()) {
      rdata = Wire.read();
      Serial.print( "[eepromReadBytes] Byte : " );
      Serial.println( rdata,HEX );
      buf[x] = rdata;
Serial.print("[eepromReadBytes] buf[x] : ");
